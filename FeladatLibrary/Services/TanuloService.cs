        ...
        public async Task<MainResponse> CreateTanulo(Tanulo tanulo)
        {
            ...
            catch (Exception ex)
            {
                ...
                if (ex.Message.Contains("409"))
                {
                    ...
                    returnResponse.ErrorMessage = "Már létezik ilyen tanuló a rendszerben!";
                }
                returnResponse.IsSuccess = false;
            ...
            try
            {
                using (var client = new HttpClient())
                {
                    ...
                    if (apiResponse.StatusCode == System.Net.HttpStatusCode.OK)
                    {
                        ...
                        if (deserilizeResponse is null) throw new NullReferenceException();
                        if (deserilizeResponse.IsSuccess)
                        {
                            returnResponse = JsonConvert.DeserializeObject<Tanulo>(deserilizeResponse.Content?.ToString() ?? string.Empty);
                            if (returnResponse is null) throw new NullReferenceException();
                        }
                    }
                }
            ...
            try
            {
                using (var client = new HttpClient())
                {
                    ...
                    if (apiResponse.StatusCode == System.Net.HttpStatusCode.OK)
                    {
                        ...
                        if (deserilizeResponse.IsSuccess)
                        {
                            returnResponse = JsonConvert.DeserializeObject<List<TanuloSync>>(deserilizeResponse.Content?.ToString() ?? string.Empty);
                        }
                    }
                }
                if (returnResponse is null) throw new NullReferenceException();
